...
from ..utils import TensorboardWriter, MetricTracker, comm
from ..utils.nn_utils import get_model_info, get_learning_rate
...
class SimpleTrainer(BaseTrainer):
    ...
    def write_metrics(self, mode, step, loss_dict: Dict, output_dict: Dict):
        """
        Write the metrics to the tensorboard and metric tracker.

        Args:
            mode (str): training mode, one of 'train' | 'valid'.
            step (int): current iteration step.
            loss_dict (dict): a dictionary of losses. Each loss item should be a tensor scalar.
            output_dict (dict): a dictionary of model's outputs. Each output item should be a numpy array.
        """
    ...
    def do_eval(self):
        ...
        for idx, data in enumerate(self.valid_loader):
            ...
            self.write_metrics('valid', (self.iter // self.eval_period - 1) * total + idx, loss_dict, output_dict)
